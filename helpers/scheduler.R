...
df$status = ifelse(df$Activity=='First pollination' & df$`Days Elapsed` > (mean_days_to_maturity+10), "Overdue",
                ifelse(df$Activity=='First pollination' & df$`Days Elapsed` %between% c(mean_days_to_maturity-10, mean_days_to_maturity+10), "Ready",
                    ifelse(df$Activity=='First pollination' & df$`Days Elapsed` %between% c(mean_days_to_maturity-30, mean_days_to_maturity-10), "Approaching",
                           ifelse(df$Activity=='First pollination' & df$`Days Elapsed` < mean_days_to_maturity-30, "Wait",
...
scheduler$`Next Activity` = ifelse(scheduler$Activity=="First pollination","Bunch Harvesting",
...
scheduler = scheduler[,c("Location", "Accession","Date", "Activity","Days Elapsed", "status", "Next Activity")]
